...
@app.get("/debug/scrapy")
def debug_scrapy_check(url: str = "https://tradingeconomics.com/calendar"):
    """Debug endpoint to test Scrapy selectors on a live URL"""
    try:
        headers = {
            'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36'
        }
        ...
        te_rows = sel.css('table#calendar tr, table.table tr')
        inv_rows = sel.css('tr.js-event-item')
        
        return {
            "status": response.status_code,
            "te_rows": len(te_rows),
            "inv_rows": len(inv_rows),
            "te_sample": [r.xpath('string(.)').get()[:50] for r in te_rows[:5]],
            "inv_sample": [r.xpath('string(.)').get()[:50] for r in inv_rows[:5]]
        }
    except Exception as e:
        return {"error": str(e)}
...
def scrape_trading_economics(date_from: str, date_to: str, country: Optional[str] = None) -> List[CalendarEvent]:
    ...
    try:
        ...
        for row in rows:
            ...
            cells = row.css('td')
            if len(cells) >= 4:
                # 0:Time, 1:Country, 2:Icon, 3:Event, 4:Actual, 5:Previous, 6:Consensus, 7:Forecast
                time_val = cells[0].xpath('string(.)').get(default="").strip()
                country_val = cells[1].xpath('string(.)').get(default="").strip()
                event_val = cells[3].xpath('string(.)').get(default="").strip() if len(cells) > 3 else cells[2].xpath('string(.)').get(default="").strip()
                ...
                event = CalendarEvent(
                    ...
                    country=country_val,
                    event=event_val,
                    actual=cells[4].xpath('string(.)').get(default="").strip() if len(cells) > 4 else "",
                    previous=cells[5].xpath('string(.)').get(default="").strip() if len(cells) > 5 else "",
                    consensus=cells[6].xpath('string(.)').get(default="").strip() if len(cells) > 6 else "",
                    forecast=cells[7].xpath('string(.)').get(default="").strip() if len(cells) > 7 else "",
...
def scrape_investing_com(date_from: str, date_to: str, country: Optional[str] = None) -> List[CalendarEvent]:
    ...
    try:
        ...
        # Investing.com uses rows with class 'js-event-item'
        ...
        for row in rows:
            ...
            try:
                ...
                event = CalendarEvent(
                    ...
                    date=event_date,
                    time=cells[0].xpath('string(.)').get(default="N/A").strip(),
                    country=cells[1].xpath('string(.)').get(default="N/A").strip(),
                    event=cells[3].xpath('string(.)').get(default="N/A").strip(),
                    actual=cells[4].xpath('string(.)').get(default="").strip(),
                    forecast=cells[5].xpath('string(.)').get(default="").strip(),
                    previous=cells[6].xpath('string(.)').get(default="").strip(),
                    impact=impact_val,
        ...
        for row in rows:
            try:
                ...
                event = CalendarEvent(
                    ...
                    time=cells[0].xpath('string(.)').get(default="N/A").strip(),
                    country=cells[1].xpath('string(.)').get(default="N/A").strip(),
                    currency=cells[1].xpath('string(.)').get(default="N/A").strip(),
                    event=event_title.strip(),
                    impact=impact,
                    actual=cells[3].xpath('string(.)').get(default="").strip(),
                    forecast=cells[4].xpath('string(.)').get(default="").strip(),
                    previous=cells[5].xpath('string(.)').get(default="").strip(),
